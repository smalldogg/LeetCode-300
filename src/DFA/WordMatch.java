...
public class WordMatch {
		...
		public void add(String word) {
				if (word != null && word.length() != 0) this.add(word, 0);
		}
		...
		public void del(String word) {
				if (word != null && word.length() != 0) this.del(word, 0 );
		}
		...
		public void del(String word, int startIdx) {
				...
				if (match != null) {
						if (++startIdx == word.length()) {
								if (match.state) {
										match.state = false;
										if (match.next.size() == 0) this.next.remove( c );
								}
						}
				} else {
						del(word, ++startIdx );
		...
		public boolean find(char[] chs, int startIdx) {
				if (startIdx >= chs.length) return false;
				char ch = chs[startIdx];
				WordMatch wordMatch = this.next.get( ch );
				if (wordMatch != null && wordMatch.state) return true;
				if (wordMatch == null) return false;
